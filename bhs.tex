...
In Section 2, we recall some general augmented Lagrangian results, where we obtain some especialized results in the linear and convex quadratic cases. In Section 3, we present our strategy for solving the augmented Lagrangian subproblem. In Section 4, we present our numerical results.\\



\noindent {\bf Notation:} The symbol $\|\cdot\|$ denotes the Euclidean norm in $\R^n$. By $\R^n_+$ we denote the set of vectors in $\R^n$ with all components non-negative. The set of non-negative integers is denoted by $\N$. If $K \subseteq \N$ is an infinite sequence of indices and $\lim_{k \in K} x^k = x$, we say that $x$ is a limit point of the sequence $\{x^k\}$.
...
We will present an augmented Lagrangin algorithm for convex quadratic programming on the lines of the developments in \cite{seco}. That is, we are interested in the following problem:
...
Our second-order Lagrange multiplier update is used when dealing with separable strictly convex quadratic problems. The update formula is obtained according to the ideas presented in \cite{yaxyuan}. For this purpose we need to introduce a notation to refer to the coordinates associated with the active box constraints at a point $x^{k+1}$, as well as with parts of vectors and matrices corresponding to these coordinates. Let $I_l=\{i: x^{k+1}_i\leq l_i\}$ and $I_u=\{i: x^{k+1}_i\geq u\}$ be the indexes of active or infeasible box constraints. Let us define $\mathcal{I}_{I_l}$  as the $|I_l|\times n$ matrix formed by the $i$-th line of the $n\times n$ identity, $i\in I_l$. Similarly we define $\mathcal{I}_{I_u}$. 
...
we consider that it only stops at an iteration $k_0$ when an exact solution is found, that is, we consider $\varepsilon=0$ in the stopping criterion. Even in this case, we consider that an inifinite sequence is generated with $x^{k}=x^{k_0}$ for all $k > k_0$.
...
there is a single compact set containing all iterands $x^k$.
...
For each function $L_i^k(x_i)$ we have
that
...
\right.
\end{equation}
it is bounded below. Thus,  there exists $\bar{L}$ such that $\bar{L} < \sum_{i=2}^n L_i^k(x_i)$.
...
The goal of our numerical experiments is to show that this method is compatible with a simple implementation of a pure interior point method. We implemented the LAQP algorithm and the interior point method for convex quadratic programming as described in \cite{gondzio25} in Julia, using ***** for solving the linear systems. \textcolor{blue}{Acho que não precisamos dizer como resolvermos os Sistemas Lineares. LRS}

Our tests are based on the Netlib \cite{Dongarra:1987jk}  test set of linear programming problems. Due to the difficulty of the problems, we use a presolved version of the collection available at~\cite{HagerCoapSoftware}. We also add a quadratic term to the objective with Hessian equal to the identity matrix, in order to formulate strictly convex separable quadratic problems. In this way, the Newtonian direction can be computed without refined linear solvers or a regularization strategy and the second-order Lagrange multiplier update is easily computable.

We start by considering all 74 (out of 98) Netlib problems with less than \num{100000} non-zero entries in the matrix of constraints. We first decide which of four possible starting points provide better results in this collection, which are the following:\\

\noindent{\tt New}: the one described in *****,\\
{\tt Ones}: the vector of ones,\\
{\tt OnesM}: the vector of ones followed by a Mehrotra-type update,\\
{\tt Mehrotra}: the usual Mehrotra point.\\

 In the case of the interior point method, the initial point is projected to a sufficient interior of the box constraints. For the Lagrange multiplier update, we use a {\tt Hybrid} approach, which uses the second-order update only when the solution of the subproblem is within the same region as the current iterate, in the sense that the corresponding Hessians of the augmented Lagrangian function coincide. Otherwise, the first-order update is used. The performance profiles, computed in terms of the number of linear systems solved, are presented below, where for each algorithm all initial points are compared.

%All initial points LAQP and IPM on the SMALL subset of Netlib (with Hybrid update?).

\includegraphics[scale=0.7]{PP_IPM-4points}

\includegraphics[scale=0.7]{PP_LAQPHybrid-4points}

Based on these results, we decide to use {\tt Mehrotra} for the interior point method initial point and {\tt New} for the augmented Lagrangian method. The performance profile is presented below.

%The same test on the SMALL subset

\includegraphics[scale=0.7]{PP_LAQPNew-IPMMehrotra}

With the same two configurations, we perform a more thorough test on all 98 Netlib problems, modified in the same way as describe before. The corresponding performance profile is the following:

%NEW initial point LAQP vs MEHROTRA IPM on all Netlib

\includegraphics[scale=0.7]{PP_LAQPNew-IPMMehrotra_All}

We enphasize that our goal is to compare a pure interior point method versus the pure augmented Lagrangian method, without sophisticated accelerations and safeguards. This explains the overall poor behavior of both methods. We have observed that IPOPT, a well tuned interior point method, solves 95\% of the Netlib problems [?]. Note also that the Netlib problems are somewhat difficult ones, as, for instance, no strict interior exists on circa 65\% of the problems [?].
...
\includegraphics[scale=0.7]{Bar_LAQPNew-IPMMehrotra_All_2}

%with the NEW initial point on LAQP, check that the second-order update is indeed better than the first-order one

We now verify the efficacy of the {\tt Hybrid} strategy for updating the Lagrange multipliers. We compare it with the {\tt Linear} update, where the first-order update is used in every iteration, and the {\tt Dual} update, where the second-order update is always used.

\includegraphics[scale=0.7]{PP_LAQP_Dual-Linear-Hybrid}

Based on these results, we conclude that the augmented Lagrangian method with the {\tt Hybrid} Lagrange multiplier update and the {\tt New} initial point had the best performance in our tests.
