...
\noindent \textbf{Key words:} Linear programming, convex quadratic programming, augmented Lagrangian, penalty methods, interior point methods\\

\noindent \textbf{AMS Subject Classification:} 90C30, 49K99, 65K05.
...
We will consider two options for the Lagrange multiplier update in Step 3. The first-order update will be defined as $\mu_l^{k+1}=\max\{0,\mu_l^k+\rho_k(-x^k+l)\}$ and $\mu_u^{k+1}=\max\{0,\mu_u^k+\rho_k(u+x^k)\}$. With this update, it has been proved in \cite{LAgnep} that the sequence $\{(\mu_l^k,\mu_u^k)\}$ is bounded. More specifically, the first-order Lagrange multiplier update is bounded for problems that satisfy the quasinormality constraint qualification (which includes linear constraints). This indicates that our update is acceptable for sufficiently large $\mu_{\max}$, hence our update coincides with the one described in the general augmented Lagrangian method \cite{bmbook}. 



Our second-order Lagrange multiplier update is used when dealing with separable strictly convex quadratic problems. The update formula is obtained according to the ideas presented in \cite{yaxyuan}. For this purpose we need to introduce a notation to refer to the coordinates associated with the active box constraints at a point $x^{k+1}$, as well as parts of vectors and matrices corresponding to these coordinates. Let $I_l=\{i: x^{k+1}_i\leq l_i\}$ and $I_u=\{i: x^{k+1}_i\geq u\}$. Let us define $id_{I_l}$  as the $|I_l|\times n$ matrix formed by the $i$-th line of the $n\times n$ identity, $i\in I_l$. Similarly we define $id_{I_u}$. 

Let $Z=\left(\begin{array}{c} -id_{I_l}\\id_{I_u}\\A\end{array}\right)$ and $M=Z(Q+H)^{-1}Z^\mathtt{T}$, where $H$ is the hessian of $$\frac{\rho_k}{2}\left(\left\|\max\left\{-x+(l+\frac{\mu_l^k}{\rho_k}),0\right\}\right\|^2+
\left\|\max\left\{x-(u-\frac{\mu_u^k}{\rho_k}),0\right\}\right\|^2\right)$$ at $x^{k+1}$, taken in the semismooth sense, namely, $H$ is a diagonal matrix with its $i$-th entry equal to $0$ if $l_i<x_i^{k+1}<u_i$ and equal to $\rho_k$ otherwise. \textcolor{red}{H é a Hessiana do pedaço que coloquei ou é sem o deslocamento mesmo? Se for com o deslocamento, a explicação de onde é zero tem que ser consertada}

When $Q$ is diagonal and positive definite (separable and stricly convex problem), the expression of $M$ can be easily computed explicitly.
...
\end{equation}
where $d^1\in\R^{|I_l|}, d^2\in\R^{|I_u|}$, and $d^3\in\R^m$. The notation $v_{I_l}\in\R^{|I_l|}$ corresponds to the vector with components $v_i, i\in I_l$ of the vector $v\in\R^n$. Similarly for $I_u$. We then define $[\mu_l^{k+1}]_{I_l}=\max\{0,[\mu_l^{k}]_{I_l}+d^1_{I_l}\}$ and $[\mu_u^{k+1}]_{I_u}=\max\{0,[\mu_u^{k}]_{I_u}+d^2_{I_u}\}$, keeping $[\mu_l^{k+1}]_i=[\mu_l^k]_i$ and $[\mu_u^{k+1}]_i=[\mu_u^k]_i$ for the remaining indexes. This is the standard second-order Lagrange multiplier update when considering that all constraints are penalized. However, by following the approach of \cite{yaxyuan} but considering constrained subproblems, we arrived at the same formula.
...
\begin{color}{red}proof of theorems... Quais???\end{color}
...
In order to analize the asymptotic behaviour of the algorithm, 
we consider in the following results the case where  we ask for the
exact solution in the stop criterion  and we declare that  $x^{k}=x^{k_0}$ for all $k \geq k_0$.

In algorithms that use an Augmented Lagrangian approach with the
standard choice to not penalize the constraints $ l \leq x \leq
u$, it is straightforward that the subproblems are well defined and
that the generated sequence remains in a compact set. The following
theorems ensure good definition of Algorithm \ref{ALalg} and also
that the generated sequence lies in a compact even with the choice
of not preserving the bound constraints. This is a consequence of dealing with quadratic convex problems.
...
When $\|x\|$ tends to infinity we have that at least one
 coordinate  of $x$ will have the absolute value as large as we want.
...
\begin{cor} \label{teobemdef} If $F \neq \emptyset$ then all the
subproblems \eqref{subprobLA} are well defined.
\end{cor}

\noindent {\it Proof.} By Lemma \ref{Lvaiinfinitosexvai}, the function $x\mapsto L(x,\rho_k,\mu_l^k,\mu_u^k)$ is coercive, and the result follows.\halmos
...
possible to ensure that the solutions of the subproblems
\eqref{subprobLA} lie on a bounded set that does not depend on $k$.

\begin{lemma} \label{ficacompato} If $F \neq \emptyset$ then the sequence
$\{x^k\}$ generated by the Algorithm \ref{ALalg} lies in a compact
set.
\end{lemma}

\noindent {\it Proof.} By Theorem \ref{teobemdef}, we have that the
subproblems \eqref{subprobLA} are well defined. Let us show that
there is a single compact set containing all the solutions of the
subproblems.

Suppose by contradiction that the sequence $\{x^k\}$ is unbounded.
...
For each function $L_i^k(x_i)$, defined in \eqref{defLik}, we have
that
...
\end{array}
\end{equation}
%and so  the function $L_i^k(x_i)$  is bounded below.


Note that $L_i(x_i)$ do not depend on $k$ and, since
...
\right.
\end{equation}
it is bounded below. Thus,  there exist $\bar{L}$ such that $\bar{L} < \sum_{i=2}^n L_i^k(x_i)$.
...
\end{equation}
and, since $\rho_k \geq \rho_0$, 
\begin{equation}\label{eq2xkcompacto}
\bar{L}+c_1x_1+ \frac{\rho_0}{2} \max\left\{x_1-u_1,0\right\}^2  < L(x,\rho_k,\mu_l^k,\mu_u^k).
\end{equation}
...
If $x_1^k > u_1+ \sqrt{2 \bar{h}}$ then 
\begin{equation}\label{eq4xkcompacto}
\frac{\rho_k}{4} \max\left\{x_1-u_1,0\right\}^2 > \frac{\rho_k}{2} \bar{h}.
\end{equation}

Given $p>0$, define  $q(x_1) = \bar{L}+c_1x_1+ \frac{\rho_0}{4} \left( x_1-u_1 \right)^2$, $\bar{C}=\bar{L}-\frac{1}{2} \bar{x}^T Q \bar{x}- c^T\bar{x} -p$, and $\Delta=c_1^2-\rho_0 \bar{C}$. Thus we have that 
\begin{equation}\label{eq5xkcompacto}
q(x_1) \geq \frac{1}{2} \bar{x}^T Q \bar{x}+ c^T\bar{x} +p\left\{
\begin{array}{l}
\text{for all } x_1 \in \R, \; \text{ if } \; \Delta  \leq 0,\\[5pt]
\text{for all } x_1 \geq \hat{x}_1 \equiv u_1+2\frac{-c+\sqrt{\Delta}}{\rho_0} \; \text{ if }  \Delta>0,
\end{array}
\right.
\end{equation}

So, if $x_1^k> \max\{u_1+ \sqrt{2 \bar{h}}, \hat{x}_1 \}$ then 
\begin{eqnarray*}
	L(x^k,\rho_k,\mu_l^k,\mu_u^k)& \geq & q(x_1)+ \frac{\rho_0}{4} \max\left\{x_1-u_1,0\right\}^2\\
	&> &  \frac{1}{2} \bar{x}^T Q \bar{x}+ c^T\bar{x} + \frac{\rho_k}{2} \bar{h}+p\\
	& \geq & L(\bar{x},\rho_k,\mu_l^k,\mu_u^k)+p.
\end{eqnarray*}
Since $p$ could be arbitrarily large, this contradicts  the fact that $x^k$ is an approximate minimizer of the subporoblem. \halmos



The next theorem guarantees that every  limit point of the sequence
generated by the algorithm converges to the solution of the original
problem \eqref{PL}.


\begin{theorem} \label{teoachasol} Assume that
the feasible set of the problem \eqref{PL} is not empty, then every
limit point of the sequence $\{x^k\}$ generated by Algorithm
\ref{ALalg} is a solution of the problem \eqref{PL}.
\end{theorem}

\noindent {\it Proof.} First, note that linear constraints satisfy
the CRCQ \cite{crcq} or even more weak constraints qualifications
such as  the CPLD \cite{cpld,rcpld}, CPG \cite{cpg} or the CCP \cite{ccp}.


By Corollary 6.2 of \cite{abms} we have that  if $x^*$ is a
...
minimizer of \eqref{probviabil}. Thus, if the original problem
\eqref{PL} has nonempty feasible set, we have that $x^*$ is feasible
for problem \eqref{PL}.

Once $x^*$ is feasible for problem \eqref{PL}, by Corollary 6.1 of
\cite{abms}, we have that $x^*$ is a stationarity point of
\eqref{PL}. By the convexity of \eqref{PL}, we have that $x^*$ is in
fact the solution of the original problem. \halmos
...
such that $x_i=l_i$ or $x_i=u_i$, respectively. The point $x$ is
said to be regular for problem \eqref{PL} if the columns of the
matrix   $[A^T \,\,\, -I^l_x \,\,\, I^u_x]$ are linearly independent.
\end{defi}

\noindent\textbf{Remark:} If  $x$ is a regular stationary point for the
...
is a regular solution of  problem \eqref{PL}  and satisfies the strict complementarity  with $\mu_l^* <
\mu_{\max}$ and $\mu_u^* < \mu_{\max}$. So, there
...
\begin{theorem} \label{convfinitaLA} Assume that the subproblems \eqref{subprobLA} are solved exactly,
	the sequence $\{x^k\}$ 
	converges to $x^*$, which
	is a regular solution of  problem \eqref{PL}  and satisfies the strict complementarity  with $\mu_l^* <
	\mu_{\max}$ and $\mu_u^* < \mu_{\max}$. So, there exists $k_1$ such
...
$x^{k_0+1}$ is a KKT point for \eqref{PL} and thus it is an solution of \eqref{PL}.
\halmos
...
\mu_{\max}$ and $\mu_u^* < \mu_{\max}$, we have, by Theorem \ref{convfinitaLA}, that the convergence is finite. \halmos
...
\begin{algorithm}[H]
  ...
  \label{solvesubp}
  \begin{algorithmic}
\State \noindent  \textbf{Step 0 (initialization)}: \\ Let $\varepsilon>0$ and set $k\leftarrow0$. Set $(x^0,\lambda^0)\in F\times\R^m$ as the current outer iterate.\\

\State \noindent \textbf{Step 1 (stopping criterion)}:\\ Set $r=Qx^k+c+A^\T\lambda^k-\rho\max\{l-x,0\}+\rho\max\{x-u,0\}$ and stop if $\|r_k\|_\infty<\varepsilon$.\\

       
    \State  \noindent \textbf{Step 2 (compute Newton direction)}:\\
    Solve the linear system
    \begin{equation}\label{newton}
    \left(\begin{array}{cc}Q+H&A^\mathtt{T}\\A&0\end{array}\right)\left(\begin{array}{c}x_{trial}\\ \lambda^{k+1}\end{array}\right)=-c+v,
    \end{equation}
    where $v_i=\rho u_i$ if $x_i\geq u_i$, $v_i=\rho l_i$, if $x_i\leq l_i$, and $v_i=0$ otherwise. Also, $H$ is diagonal with $H_{ii}=0$ if $l_i<x_i<u_i$, and $H_{ii}=\rho$, otherwise.
    \State  \noindent \textbf{Step 3 (line search)}:\\
Set $d_x=\gamma(x_{trial}-x^k)$, where $\gamma=\min\left\{1,\frac{100}{\|x_{trial}-x^k\|}\right\}$. Find $t^*$ as the global minimizer of the one-dimensional piecewise quadratic $t\mapsto L(x^k+td_x)$ and reset $t^*$ as the projection of $t^*$ onto the safeguarded interval $[-\min\{\|d_x\|,10^{-6}\},1+\|d_x\|]$.\\

    \State  \noindent \textbf{Step 4. (update and repeat)}: Set $x^{k+1}=x^k+t^*d_x$. Update $k \leftarrow k+1$ and go to Step~1.\\

  \end{algorithmic}
\end{algorithm}

The linear system \eqref{newton} is the standard Newton direction for the KKT system. The direction is re-scaled to avoid a direction with norm greater than $100$. The minimization of $L(x^k+td_x)$ is done by a sequence of straightforward minimization of (smooth) unidimensional convex quadratics. The safeguarded projection is considered to avoid numerical errors when the Newton direction is not accurately computed.
...
Once again, in order to analyze the asymptotic behavior, we will consider that  Algorithm~\ref{algsubprob} stops only if there is $k_0$ such that $x^{k_0}$ is an exact
...
The goal of our numerical experiments is to show that this method is comparable with a simple implementation of a pure interior point method. We implemented the LAQP algorithm (called \texttt{LAPQ}) and the interior point method for convex quadratic programming as described in~\cite{gondzio25} (called \texttt{IPM}) in Julia~\cite{Bezanson:2017g}. We used subroutine MA57~\cite{Duff:2004cx} from HSL~\cite{HSL:7NBcL3Rm} to solve the augmented linear systems that arises in both methods. 
 

Our tests are run on the \textsc{Netlib}~\cite{Dongarra:1987jk} test collection of linear programming problems. Due to the difficulty of the problems, we use a presolved version of the collection available at~\cite{HagerCoapSoftware}. We also add a quadratic term to the objective function with Hessian equal to the identity matrix, in order to obtain strictly convex separable quadratic problems. In this way, the Newtonian direction can be computed without refined linear solvers or a regularization strategy and the second-order Lagrange multiplier update is easily computable.
...
\begin{description}
  \item[\texttt{New}:] in this novel approach $x^0$ is computed as the projection of the solution of   $\min \frac{1}{2}x^{T}Qx + c^Tx $, subject to $Ax=b$ into the box constraints. That is to say we are computing a solution of \eqref{PL} without the box constraints and then projecting it into those same restrictions. Next, we compute $\lambda^{0}$ as the solution of $A^{T}\lambda = -(Qx^{0}+c)$ and we define  $\mu_l^0=-\min\{0, \mu\}$  and
$\mu_u^0=\max\{0,\mu\}$, where $\mu = -(Qx^{0}+c+A^{T}\lambda^{0})$;
...
\item [\texttt{OnesM}:] $x^{0}$ is the vector of ones and $\lambda^{0}$, $\mu_l^0$  and $\mu_u^0$ are computed  by a Mehrotra-type update.


\end{description}

Independently of the choice above, the second step depends on the method: for \texttt{LAQP}, we perform a full Newton step to recover feasibility regarding $Ax=b$; for  \texttt{IPM}, we use the shifting strategy implemented by Mehrotra~\cite{Mehrotra:1992wr} to assure that the initial point is (sufficiently) in the interior of the box constraints. 
...
%All initial points LAQP and IPM on the SMALL subset of Netlib (with Hybrid update?).
...
With the same two configurations, that is, \texttt{IPM-Mehrotra} and \texttt{LAQPHybrid-New}, we perform a more thorough test on all 95 available presolved \textsc{Netlib} problems,\textbf{modified in the same way as describe before.} The corresponding performance profile is on \Cref{fig:PP_LAQPNew-IPMMehrotra_All}.

%NEW initial point LAQP vs MEHROTRA IPM on all \textsc{Netlib}
\begin{figure}[!ht]\centering
\includegraphics[width=.72\textwidth]{PP_LAQPNew-IPMMehrotra_All}
\caption{Performance profiles of \texttt{IPM-Mehrotra} and \texttt{LAQPHybrid-New} for all 95 available \textsc{Netlib} problems.}\label{fig:PP_LAQPNew-IPMMehrotra_All}
\end{figure}


We emphasize that our goal is to compare a pure interior point method versus the pure augmented Lagrangian method proposed, without sophisticated accelerations and safeguards. This explains the overall mildly poor performance of both methods. For instance, as indicated  by~\textcite{Haeser:2017wl},  IPOPT~\cite{Wachter:2005hk}, a well tuned interior point method, solves 56 of the 68 smallest problems of  \textsc{Netlib} collection, when limited to 300 iterations.  Furthermore,  the \textsc{Netlib} problems are rather difficult ones, as, for instance, no strict interior point exists on circa of 65\% of the problems \cite{Haeser:2017wl}.

To analyze the behavior of both methods in more details, the figure below represents each problem on the horizontal axis, ordered in increasing size of non-zero entries in the constraints. An asterisk is marked when none of the solvers found a solution. A bar with a positive vertical coordinate indicates that LAQP was the best solver in terms of the number of linear systems solved, while a negative coordinate indicates that the interior point method was better. The size of the bar indicates ******. More precisely, the vertical coordinate is computed as *****.
