...
The goal of our numerical experiments is to show that the Augmented Lagrangian method proposed is compatible with a simple implementation of a pure interior point method. We implemented Algorithm \ref{ALalg} (called \texttt{LAQP}) and the interior point method for convex quadratic programming as described in~\cite{gondzio25} (called \texttt{IPM}), both in Julia~\cite{Bezanson:2017g}. We used subroutine MA57~\cite{Duff:2004cx} from HSL~\cite{HSL:7NBcL3Rm} to solve the augmented linear systems that arise in both methods. We run the tests on an iMac Pro with Intel Xeon W 3.2 GHz Processor and 32 GB RAM.
...
Based on these results, we decide to use \texttt{Mehrotra}  as initial point for \texttt{IPM} and \texttt{New} for \texttt{LAQP}. The performance profile comparing those two results is presented in \Cref{fig:PP_LAQPHybridNew-IPMMehrotra}.
...
We emphasize that our goal is to compare a pure interior point method versus the pure augmented Lagrangian method proposed, without sophisticated accelerations and safeguards. This explains the overall poor behavior of both methods. As pointed out in~\cite{Haeser:2017wl}, IPOPT~\cite{Wachter:2005hk}, a well tuned interior point method, solves 56 of the 68 smallest problems of the \textsc{Netlib} collection when limited to 300 iterations. They also show that no strict interior point exists on circa 65\% of \textsc{Netlib} problems, which indicates that those  are rather difficult problems.
...
\caption{Number of linear systems solved before and after perturbation on $x^{0}$ for \texttt{LAQP} and \texttt{IPM}.}
\label{tab:LAQP_IPM_Perturbed_x}
\begin{tabular}{>{\ttfamily}lS[table-format=5,group-minimum-digits=3]cccc}
\toprule
{\normalfont \textbf{Problem}}  & {\textbf{Nonzeros}} &\texttt{LAQP} &   \texttt{IPM} & \texttt{LAQP-Perturbed}  & \texttt{IPM-Perturbed} \\ 
\midrule  
afiro    & 88   &   16          &   13   &   \textbf{2}      &  xx  \\
adlittle & 465  &   61          &   17   &   \textbf{2}      &  xx  \\
agg2     & 4515 &   64          &   34   &   \textbf{2}      &  xx  \\
agg3     & 4531 &   61          &   33   &   \textbf{2}      &  xx  \\
beaconfd & 3476 &    \textbf{2} &   15   &   \textbf{2}      &  xx  \\
blend    & 521  &   22          &   16   &   \textbf{2}      &  xx  \\
bore3d   & 1525 &   36          &   38   &  19               &  xx  \\
e226     & 2767 &   73          &   28   &  61               &  xx  \\
ganges   & 7021 &  152          &   28   &  44               &  xx  \\
grow15   & 5665 &   \textbf{76} &   79   &   \textbf{2}      &  xx  \\
grow22   & 8318 &   \textbf{40} &   84   &   \textbf{2}      &  xx  \\
grow7    & 2633 &   \textbf{71} &   80   &   \textbf{2}      &  xx  \\
israel   & 2358 &  \textbf{179} &  132   &  77               &  xx  \\
kb2      & 291  &  108          &   20   &  89               &  xx  \\
lotfi    & 1086 &   \textbf{14} &   17   &   \textbf{2}      &  xx  \\
modszk1  & 4158 &  294          &   50   &  24               &  xx  \\
qap8     & 8304 &  131          &   16   & --                &  xx  \\
sc105    & 281  &    \textbf{8} &   21   &   \textbf{2}      &  xx  \\
sc205    & 552  &   \textbf{16} &   62   &   \textbf{2}      &  xx  \\
sc50a    & 131  &    \textbf{2} &    7   &   \textbf{2}      &  xx  \\
sc50b    & 119  &    \textbf{4} &   10   &   \textbf{2}      &  xx  \\
scrs8    & 4029 &  195          &   31   &  10               &  xx  \\
scsd6    & 5666 &   \textbf{58} &   92   &  62               &  xx  \\
sctap1   & 2052 &   69          &   45   &   \textbf{3}         xx  \\
sctap2   & 8124 &  126          &   38   &  38               &  xx  \\
seba     & 4874 &    \textbf{9} &   12   &   \textbf{2}      &  xx  \\
share2b  & 730  &   43          &   37   &   5               &  xx  \\
stair    & 3857 &   81          &   59   &   \textbf{2}      &  xx  \\
stocfor1 & 474  &   15          &   13   &  46               &  xx  \\
\bottomrule
\end{tabular}
\end{table}
...
In \Cref{tab:LAQP_IPM_Perturbed_x} we can see that in $17$ problems ($59$\%), \texttt{LAQP} found the solution faster than \texttt{IPM}. In this cases, \texttt{LAQP} took in average $2$ additional iterations, while \texttt{IPM} took, in average, $4.8$ iterations, excluding three cases where more than $40$ additional iterations were necessary. In $1$ problem both methods took $5$ additional iterations and in the remaining $11$ problems ($38$\%) where \texttt{IPM} found the solution faster than \texttt{LAQP}, we can see that, in general, these were cases where in the original run \texttt{LAQP} was already inferior to \texttt{IPM}. Also, in $12$ problems where \texttt{LAQP} was originally faster than \texttt{IPM}, \texttt{LAQP} was still faster to recover the solution on the second run on $10$ problems ($83$\%), while on $16$ problem where \texttt{IPM} was faster on the first run, \texttt{IPM} was able to be faster to recover the solution on the second run on $9$ problems ($56$\%).

We consider again this collection of $29$ problems where both methods found a solution and their corresponding solutions found, say, $x^*$. We now perform two tests by using the solution of this problem as initial points to a perturbed problem. The first test consists of perturbing the problem in the following way: We find the first index $i$ of $x^*$ such that $l_i=[x^*]_i$ and we then redefine $l_i\leftarrow l_i+1$. Now $x^*$ is infeasible but it can be used as the initial point for \texttt{LAQP}. For \texttt{IPM} we use $x^*$ followed by a Mehrotra shift as the initial point. The second test consists of finding the index of the largest interior component of $x^*$, that is, $l_i<[x^*]_i<u_i$. For this index we re-define $u_i$ as the largest integer smaller than $[x^*]_i$. In both tests we consider appropriate safeguards to ensure $l\leq u$. The results are reported in Tables \ref{tab:LAQP_IPM_Perturbed_l} and \ref{tab:LAQP_IPM_Perturbed_u}.




\begin{table}[!hbt]\centering
\caption{Number of systems solved after perturbation on $x^{0}$ for \texttt{LAQP} and \texttt{IPM}.}
\label{tab:LAQP_IPM_Perturbed_lu}
\begin{tabular}{>{\ttfamily}lcccc}
\toprule
{\normalfont \textbf{Problem}} & \texttt{LAQP-Perturbed-l}   & \texttt{IPM-Perturbed-l} & \texttt{LAQP-Perturbed-u}   & \texttt{IPM-Perturbed-u} \\ 
\midrule  
afiro    &   7 &    8 &    2 &    8   \\
adlittle &   2 &    5 &    2 &   10   \\
agg2     &   2 &    4 &    2 &   15   \\
