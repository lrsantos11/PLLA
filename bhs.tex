...
Since in this approach there are no equality constraints to be penalized, we will use a slightly different notation than the one presented in the general algorithm. We denote by $\lambda\in\R^m$ a Lagrange multiplier approximation associated with constraints $x\in F$, and by $\mu_l\in\R^n_+$ and $\mu_u\in\R^n_+$ the Lagrange multipliers approximations associated with constraints $-x+l\leq 0$ and $x-u\leq 0$, respectively. So, given $\rho$, $\mu_l$  and $\mu_u$, the Augmented Lagrangian function used in this paper is defined by
\begin{equation} \label{auglag}
L(x,\rho,\mu_l,\mu_u)=\frac{1}{2}x^TQx+c^Tx+\frac{\rho}{2}\left(\left\|\max\left\{-x+\left(l+\frac{\mu_l}{\rho}\right),0\right\}\right\|^2+ \left\|\max\left\{x-\left(u-\frac{\mu_u}{\rho}\right),0\right\}\right\|^2\right),
\end{equation}
and the subproblem of interest is
\begin{equation} \label{subprobLA}
\text{Minimize } \, L(x,\rho,\mu_l,\mu_u) \, \text{ subject to } \, Ax=b,
\end{equation}
whose optimality conditions are $\nabla L(x,\rho,\mu_l,\mu_u)+ A^T \lambda=0$ and $Ax=b$. Therefore we say that a  point $x \in F$ is an $\epsilon$ approximate solution of \eqref{subprobLA} if there exists $\lambda$ such that  $\|\nabla L(x,\rho,\mu_l,\mu_u)+ A^T \lambda \| \leq \epsilon$.  Precisely, the algorithm is set out as follows.
...
\text{Minimize } \,  L(x,\rho_k,\mu_l^k,\mu_u^k) \, \text{ subject to } \, Ax=b.
\end{equation}
    ...
    $$\left\|\max\left\{-\frac{\mu_l^k}{\rho_k},-\frac{\mu_u^k}{\rho_k},x^{k+1}-u,l-x^{k+1}\right\}\right\|_{\infty}>
    \frac{1}{2}\left\|\max \left\{-\frac{\mu_l^{k-1}}{\rho_{k-1}},-\frac{\mu_u^{k-1}}{\rho_{k-1}},x^{k}-u,l-x^{k} \right\}\right\|_{\infty}$$ set $\rho_{k+1}=10 \rho_k,$ otherwise set $\rho_{k+1}=\rho_k$.\\
    	%,-\frac{\mu_u^{k-1}}{\rho_{k-1}},x^{k}-u,l-x^{k}
...
We will consider two options for the Lagrange multiplier update in Step 3 of Algorithm \ref{ALalg}. The first-order update will be defined as $\mu_l^{k+1}=\max\{0,\mu_l^k+\rho_k(-x^k+l)\}$ and $\mu_u^{k+1}=\max\{0,\mu_u^k+\rho_k(u+x^k)\}$. This classic rule correspond to make a steepest descent iteration for the dual function, see \cite{bertsekasCOA}. Using this update in all iterations, it has been proved in \cite{LAgnep} that the sequence $\{(\mu_l^k,\mu_u^k)\}$ is bounded. More specifically, the first-order Lagrange multiplier update is bounded for problems that satisfy the quasinormality constraint qualification (which includes linear constraints). This indicates that this update is acceptable for sufficiently large $\mu_{\max}$, hence this parameter can be dropped.



Our second-order Lagrange multiplier update is used when dealing with separable strictly convex quadratic problems. The update formula is obtained according to the ideas presented in \cite{yaxyuan,bertsekas,buys}. For the derivation of this rule it is used that, fixing $\rho$, the solution of subproblem  \eqref{subprobLA} is unique for each $\mu_l$ and $\mu_u$, and so we can define $x(\mu)$ as the solution to this problem. Thus, the second order update of the multipliers consists of taking the positive part of the solution of applying one iteration of Newton's method to maximize the associated dual function \cite{bertsekas,buys} or, in accordance with \cite{yaxyuan}, to the  system defined by the constraints in order to approach feasibility.

With the purpose of presenting the update formula  we need to introduce a notation to refer to the coordinates associated with the active or infeasible displaced box constraints at a point $x$.  Thus let us define $\alpha_l^k(x)=\left\{i: x_i\leq l_i+\frac{[\mu_l^k]_i}{\rho_k}\right\}$, $\alpha_l^k=\alpha_l^k(x^{k+1})$, $\alpha_u^k(x)=\left\{i: x_i\geq u_i-\frac{[\mu_u^k]_i}{\rho_k}\right\}$ and $\alpha_u^k=\alpha_u^k(x^{k+1})$. The coordinates related to the constraints without the displacement will be denoted without the indexes $k$, that is, $\alpha_l(x)=\left\{i: x_i\leq l_i \right\}$ and $\alpha_u(x)=\left\{i: x_i\geq u_i \right\}$.



Moreover, we will need to refer to parts of vectors and matrices corresponding to these coordinates.
 Let us define $\mathcal{I}_l^k$  as the $|\alpha_l^k|\times n$ matrix formed by the $i$-th line of the $n\times n$ identity, $i\in \alpha_l^k$. In the same way we define $\mathcal{I}_u^k$.  The notation $v_{\alpha_l^k}\in\R^{|\alpha_l^k|}$ corresponds to the vector with components $v_i, i\in \alpha_l^k$ of the vector $v\in\R^n$. Similarly for $\alpha_u^k$. Since the Newton method is a local procedure, at  $x^{k+1}$,  $\mu_l^k$, $\mu_u^k$, and $\rho_k$, the formula would be the same as the positive part of the one applied to the problem
 \begin{equation} \label{QPRestAtiv}
 \text{Minimize } \frac{1}{2}x^TQx+c^Tx \text{ subject to } Ax=b, \;\;\; x_{\alpha_l^k}=l_{\alpha_l^k}, \, \text{ and } \, x_{\alpha_u^k}=u_{\alpha_u^k},
 \end{equation}
 when penalizing the active box constrains.
 
 Let $Z_k=\left(\begin{array}{c} -\mathcal{I}_l^k\\\mathcal{I}_u^k\\A\end{array}\right)$ and $M_k=Z_k(Q+H^k)^{-1}Z_k^\mathtt{T}$, where $H^k$ is the Hessian of $$\frac{\rho_k}{2}\left(\left\|\max\left\{-x+\left(l+\frac{\mu_l^k}{\rho_k}\right),0\right\}\right\|^2+
\left\|\max\left\{x-\left(u-\frac{\mu_u^k}{\rho_k}\right),0\right\}\right\|^2\right)$$ at $x^{k+1}$, taken in the semismooth sense, namely, $H^k$ is a diagonal matrix with its $i$-th entry equal to $0$ if $l_i+\frac{[\mu_l^k]_i}{\rho_k}<x_i^{k+1}<u_i-\frac{[\mu_u^k]_i}{\rho_k}$ and equal to $\rho_k$ otherwise. To update the Lagrange multiplier approximations $\mu^k_l$ and $\mu^k_u$, we solve the linear system \begin{equation}\label{sist2orderupdate}
M_k\left(\begin{array}{c}d^1\\d^2\\d^3\end{array}\right)=\left(\begin{array}{c}l_{\alpha_l^k}-x^{k+1}_{\alpha_l^k}\\x^{k+1}_{\alpha_u^k}-u_{\alpha_u^k}\\b-Ax^{k+1}\end{array}\right),
\end{equation}
where $d^1\in\R^{|\alpha_l^k|}, d^2\in\R^{|\alpha_u^k|}$, and $d^3\in\R^m$.  We then define $[\mu_l^{k+1}]_{\alpha_l^k}=\max\{0,[\mu_l^{k}]_{\alpha_l^k}+d^1\}$ and $[\mu_u^{k+1}]_{\alpha_u^k}=\max\{0,[\mu_u^{k}]_{\alpha_u^k}+d^2\}$, keeping $[\mu_l^{k+1}]_i=[\mu_l^k]_i$ and $[\mu_u^{k+1}]_i=[\mu_u^k]_i$ for the remaining indexes. This is the standard second-order Lagrange multiplier update when considering that all constraints are penalized. However, by following the approach of \cite{yaxyuan} but considering constrained subproblems, we arrived at the same formula.

When $Q$ is diagonal and positive definite (separable and stricly convex problem), the expression for $M_k$ can be easily computed explicitly, and this is the case where we may use the second-order update. If $Q$ is not positive definite or the inverse of $Q+H^k$ is not easily computable, we advocate the use of the first-order update only. In our numerical  implementation we also used the first-order update if the solution of \eqref{sist2orderupdate} is not accurate.
...
In \cite{yaxyuan} the author presented a local superlinear convergence for the second order update rule,  unfortunately, he said that he was not able to obtain an general result. Considering this, we tested our algorithm using the second-order update with safeguards in all iterations and also making use of this formula only when we had some indication that we were approaching the solution. The most efficient version of our algortihm uses the first-order update unless two consecutive iterations are such that  $\alpha_l^{k-1}(x^{k-1})=\alpha_l^{k}(x^{k})$ and $\alpha_u^{k-1}(x^{k-1})=\alpha_u^{k}(x^{k})$. 

Since we will prove that the sequence $x^k$ converges to $x^*$, a solution of the problem \eqref{PL}, it is reasonable to expect  that the number of iterations of the method in which second-order updates with $\alpha_l^{k}(x^{k}) \neq \alpha_l(x^{*})$ or  $\alpha_u^{k}(x^{k}) \neq \alpha_u(x^{*})$ are made are finite. Therefore, the behavior of the hybrid method must be very similar with the method that uses the standard first-order update  until $\alpha_l^{k}(x^{k}) = \alpha_l(x^{*})$ and  $\alpha_u^{k}(x^{k}) = \alpha_u(x^{*})$.
...
When solving the subproblem, the indexes $k$ of $\rho_k$, $\mu_l^k$ , $\mu_u^k$, and $\varepsilon_{k+1}$
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\State \noindent \textbf{Step 1 (stopping criterion)}:\\ Set $r=Qx^k+c+A^\T\lambda^k-\rho\max\{l-x^k,0\}+\rho\max\{x^k-u,0\}$ and stop if $\|r\|_\infty\leq\varepsilon$.\\
		
		
		\State  \noindent \textbf{Step 2 (Regularization)}:\\
		Define $H^k$ as the diagonal matrix such that $H^k_{ii}=0$ if $l_i<x_i^k<u_i$, and $H_{ii}^k=\rho$, otherwise. If $\left(\begin{array}{cc}Q+H^k&A^\mathtt{T}\\A&0\end{array}\right)$ is singular, set $ H^k=H^k+\varepsilon_{reg}I$.\\
		...
		\end{equation}
		where $v_i^k=\rho u_i$ if $x_i^k\geq u_i$, $v_i^k=\rho l_i$, if $x_i^k\leq l_i$, and $v_i^k=0$ otherwise.
...
Once again, in order to analise the asymptotic behaviour, we will consider that the Algorithm~\ref{solvesubp} stops only if there is $k_0$ such that $x^{k_0}$ is an exact
...
Since $L(x)$ is continuos and coercive, we have that $\{L(x^k)\}$ is bounded below, so
...
\begin{theorem}\label{convfinitasubprob}
	Supose that the subproblem \eqref{subprobLAk} has a unique solution $x^*$ and that
	$l_i \neq x^*_i \neq u_i$ for all $i$,	then  $x^{k}=x^*$ for $k$ large enough.
\end{theorem}

\noindent {\it Proof}  Since $l_i \neq x^*_i \neq u_i$ for al $i$, $L(x)$ is a convex smooth quadratic function $q(x)$ in a neiborhood of $x^*$. Since \eqref{subprobLAk} has an unique solution, $x^*$ is also the unique solution of the quadratic problem 
...
By Theorem \ref{ptolimitesolucao} we have that the whole sequence $\{x^k\}$ converges to $x^*$, so, for $k$ large enough, $Q+H^k=\nabla^2 L(x^k)$. Thefore, in this situation, $d^k$ is the Newton step for a strictly convex quadratic problem, and so $x^{k+1}=x^k+d^k$ is the solution of \eqref{subprobLAk}. 
\halmos
...
We start  showing the finite convergence for LP problems under mild hipotheses. After that,  we present some algorithmic hypotheses that will be used in the finite convergence analysis  when dealing with strictly convex quadratic programming  and using the second order Lagrange multipliers update is used. We will discuss the plausibility of  the assumptions showing that, under certain conditions, they are  reasonable. 
...
\begin{hip}\label{hipsubprobresolvido}
	...
	$$\|\nabla L(x^{k+1},\rho_k,\mu_l^k,\mu_u^k)+ A^T \lambda^{k+1} \| \leq \nu_{k+1} \left\|\max\left\{-\frac{\mu_l^k}{\rho_k},-\frac{\mu_u^k}{\rho_k},x^{k+1}-u,l-x^{k+1}\right\}\right\|,$$
	where $\nu_{k} \to 0$.
\end{hip}
...
\noindent {\it Proof.} 
Consider the following system whith $n+m+|\alpha_u(x^*)|+|\alpha_l(x^*)|$ variables and equations
\begin{equation}\label{sistKKTcompcertas}
Qx+c+A^T \lambda- \mu_l+\mu_u=0, \,\, Ax=0, \,\, x_{\alpha_u(x^*)}=u_{\alpha_u(x^*)}, \, \text{ and } \, x_{\alpha_l(x^*)}=u_{\alpha_l(x^*)}.
\end{equation}
Under Assumption \ref{hipregcompestrita} and \ref{hipnondegene}, if $Q = 0$, we   have that the Jacobian of \eqref{sistKKTcompcertas} is nonsingular. This means that Assumption 7.4 of \cite{bmbook} holds. Thus all the hipotheses of Theorem 7.2 of \cite{bmbook} are satisfied and so it   guarantees the existence of $\bar{\rho}$.  \halmos
...
\begin{lemma} \label{LemaCompcertas} Let $\{x^k\}$, $\{\mu^{k}_l\}$, and $\{\mu^{k}_u\}$ be sequences generated by Algorithm \ref{ALalg} when solving problem \eqref{PL} with $Q=0$ and using the first order update of the Lagrange multipliers. Supose that  Assumptions \ref{hipregcompestrita}, \ref{hipnondegene} and \ref{hipsubprobresolvido} hold, then there
exists $k_0 \in \N$ such that, for all $j \geq k \geq k_0$, $\alpha_l^{k}(x^j) =\alpha_l (x^*)$ and $\alpha_u^k(x^j)=\alpha_u (x^*)$.
\end{lemma}

\noindent {\it Proof.}  Once again we have that the hypoteses of Lemmas 7.1 and 7.2 and Theorem 7.2 of \cite{bmbook} holds. Therefore, $\lim_{k \to \infty} \mu^k_l=\mu^*_l$, $\lim_{k \to \infty} \mu^k_u=\mu^*_u$, $\mu^{k}_l>0 \Leftrightarrow
\mu^*_l>0$, $\mu^{k}_u > 0 \Leftrightarrow \mu^*_u>0$, and $\rho_k = \bar{\rho}$ for $k$ large enough. In this case, by the strictly complementarity, we have that for $k$ large enough  $\frac{[\mu^{k}_l]_i}{\rho_{k}}$ and $\frac{[\mu^{k}_u]_i}{\rho_{k}}$ are bounded away from zero if $x_i^*=l_i$ or  $x_i^*=u_i$. Since  $\{x^j\}$ 
converges to $x^*$, we have that $x^{j}_i<l_i+\frac{[\mu^{k}_l]_i}{\rho_{k}}\Leftrightarrow
x^*_i=l_i$ and
$x^{j}_i>u_i-\frac{[\mu^{k}_u]_i}{\rho_{k}}\Leftrightarrow
x^*_i=u_i$ for $k$ and $j$ large enough. \halmos

  Next lemma show that the exact Lagrange multipliers are obtained when dealing with Linear Programming problems. By Lemma \ref{LemaCompcertas}, the result could be obtained following directcly from Proposition 5.2.1 of \cite{bertsekasCOA} considering the equality constrained subproblem fixing $x_{\alpha_l(x^*)}=l_{\alpha_l(x^*)}$ and $x_{\alpha_u(x^*)}=u_{\alpha_u(x^*)}$ and penalizing theses constraints. However, we chose to present a new proof because it is more direct and constructive, without the use of the concepts of proximal method. Note that with the strict complementarity hypothesis, the boundedness of the penalty parameter and the identification of the active constraints, the hypotheses of Theorem~\ref{convfinitasubprob} are satisfied and so it is natural to assume that the subproblems are solved exactly for $k$ sufficiently large.

\begin{lemma} \label{convfinitaLAPL} Let $k_0$ be defined as in Lemma \ref{LemaCompcertas} and $\{x^k\}$, $\{\mu^{k}_l\}$, and $\{\mu^{k}_u\}$ be sequences generated by Algorithm \ref{ALalg} when solving problem \eqref{PL} with $Q=0$ and using the first order update of the Lagrange multipliers. Supose that  Assumptions \ref{hipregcompestrita} and \ref{hipsubprobresolvido} hold and that subproblems \eqref{subprobLAk} are solved exactly for $k \geq k_0$  ($\varepsilon_k=0$), then   there exists $k_1$ such that $\mu^{k_1}_l=\mu_l^*$ and  $\mu^{k_1}_u=\mu_u^*$.
\end{lemma}
...
\begin{theorem} \label{convfinitaLA2} Let $k_0$ be defined as in Lemma \ref{LemaCompcertas} and $\{x^k\}$ be   generated by Algorithm \ref{ALalg} when solving problem \eqref{PL} with $Q=0$ and using the first order update of the Lagrange multipliers. Supose that  Assumptions \ref{hipregcompestrita} and \ref{hipsubprobresolvido} hold and that subproblems \eqref{subprobLAk} are solved exactly for $k \geq k_0$, then there exists $k_2$ such 	that $x^{k_2}=x^*$.
\end{theorem}


\noindent {\it Proof.}  
Let $k_1$ be defined as in Lemma \ref{convfinitaLAPL} such that  $\mu^{k_1}_l=\mu_l^*$ and  $\mu^{k_1}_u=\mu_u^*$. Since $x^{k_1+1}$ is a solution of the linearly constrained problem \eqref{subprobLAk}, there exists $\lambda^{k_1+1} \in \R^m$ such
that $(x^{k_1+1},\lambda^{k_1+1})$ satisfies the system
\begin{equation} \label{KKTemK0mais1}
\begin{array}{c}
c+A^T \lambda^{k_1+1}- \rho_{k_1}\max\left\{l-x^{k_1+1}+\frac{\mu^*_l}{\rho_{k_1}},0\right\}+
...
$x^{k_1+1}_i=l_i \Leftrightarrow x^*_i=l_i$ and $x^{k_1+1}_i=u_i \Leftrightarrow x^*_i=u_i$. So
...
Furthermore, the hypoteses of Lemmas 7.1 and 7.2  of \cite{bmbook} holds whith the first order update rule. This means that, under Assumptions \ref{hipregcompestrita} and \ref{hipsubprobresolvido}, the algorithm method with the first-order update generates sequences that converge to exact multipliers. 
Thus, in the same way as in Lemma \ref{LemaCompcertas}, using the strictly complementarity and the boundedness of $\rho_k$, we can prove that the algorithm appling the first order update to strictly convex problems correctly identifies the active constraints for $k$ sufficiently large. This suggests that plausibility of Assumption \ref{hipregcerta} for the sequences generated by the hybrid method.

\begin{hip}\label{hipregcerta}
	There 	exists $k_0 \in \N$ such that, for all $j \geq k \geq k_0$, $\alpha_l^{k}(x^j) =\alpha_l (x^*)$ and $\alpha_u^k(x^j)=\alpha_u (x^*)$.
\end{hip}
...
\begin{lemma} \label{convfinitaLAQP} Consider that problem \eqref{PL} is defined with $Q$ positive definite and that $\{x^k\}$, $\{\mu^{k}_l\}$, and $\{\mu^{k}_u\}$ are sequences generated by Algorithm \ref{ALalg} using the hybrid Lagrange multipliers update when solving it. Supose that  Assumptions \ref{hipregcompestrita} and \ref{hipregcerta} hold and that subproblems \eqref{subprobLAk} are solved exactly for $k \geq k_0$, $k_0$ defined at Assumption \ref{hipregcerta}. Then, there exists $k_1 \geq k_0$ such 	that $\mu^{k_1}_l=\mu_l^*$ and $\mu^{k_1}_u=\mu_u^*$.
\end{lemma}

\noindent {\it Proof.}  
Note that because the subproblem is picewise quadratic  and Newton's method uses only local information, the trial point $x^{k+1}_{trial}$ is the solution of the smooth quadratic problem
\begin{equation}\label{subprobsuave}
 \min_x L^k(x, \mu^k) \text{ s.t. } Ax=b,
 \end{equation}
where
$$L^k(x, \mu) = \frac{1}{2}x^TQx+c^Tx+\sum_{i \in \alpha_l^k(x^k) } \left(l_i-x_i+\frac{[\mu_l]_i}{\rho_k}\right)^2 +\sum_{i \in \alpha_u^k(x^k) } \left( x_i-u_i+\frac{[\mu_u]_i}{\rho_k}\right)^2.$$
...
$$\min \, \bar{g}_k(t)=L(x^k+t(x^{k+1}_{trial}-x^k),\rho_k,\mu_l^k,\mu_u^k).$$
Since $\bar{g}_k(t)$ has first order derivatives we have that $\bar{g}_k'(t^k)=0$. Using the chain rule once again, we got that    $\nabla L(x^{k+1},\rho_k,\mu_l^k,\mu_u^k)^T(x^{k+1}_{trial}-x^k)=0$. If  $k_0$ is such that $\alpha_l^{k_0}(x^{{k_0}+1})=\alpha_l^{{k_0}}(x^{k_0})$ and $\alpha_u^{k_0}(x^{{k_0}+1})=\alpha_u^{{k_0}}(x^{k_0})$ then $\nabla L(x^{{k_0}+1},\rho_{k_0},\mu_l^{k_0},\mu_u^{k_0})= \nabla L^{k_0}(x^{{k_0}+1}, \mu^{k_0})$ and so  $g'_{k_0}(t^{k_0})=0$. Since $g_{k_0}(t)$ is a strictly convex function, its minimizer is unique and therefore $t^{k_0}=1$, which means that $x^{{k_0}+1}=x^{{k_0}+1}_{trial}$.


 Using the approach of \cite{yaxyuan}, note that by Assumption \ref{hipregcerta}, the second-order update consists of applying Newton's method to obtain $x(\mu)_{\alpha_l(x^*)} = l_{\alpha_l(x^*)}$ and $x(\mu)_{\alpha_u(x^*)} = u_{\alpha_u(x^*)}$, where $x(\mu)$ is, for a fixed $\rho_k$, the unique  solution of \eqref{subprobLA}  given $\mu_l$ and $\mu_u$. Since $x^{{k_0}+1}=x^{{k_0}+1}_{trial}$ we have that $x(\mu^{k_0})=x^{{k_0}+1}$. Since $x(\mu)$ is linear, the Newton's method finds the exact solution in a single iteration. This means that $(\mu_l^{{k_0}+1},\mu_u^{{k_0}+1})=(\mu_l^*,\mu_u^*)$.
\halmos

Once the exact Lagrange multipliers are obtained, the primal solution is also found.

\begin{theorem} \label{convfinitaLAQP2} Consider that problem \eqref{PL} is defined with $Q$ positive definite and that $\{x^k\}$ is generated by Algorithm \ref{ALalg} using the hybrid Lagrange multipliers update when solving it. Supose that  Assumptions \ref{hipregcompestrita} and \ref{hipregcerta} hold and that subproblems \eqref{subprobLAk} are solved exactly for $k \geq k_0$, $k_0$ defined at Assumption \ref{hipregcerta}. Then, there exists $k_2$ such 	that $x^{k_2}=x^*$.
\end{theorem}


\noindent {\it Proof.}  
By Lemma \ref{convfinitaLAQP}, there exists $k_1 \geq k_0$ such that  $\mu^{k_1}_l=\mu_l^*$ and $\mu^{k_1}_u=\mu_u^*$. Therefore $x^{k_1+1}$ is a KKT point  of the subproblem and so  there exists $\lambda^{k_1+1} \in \R^m$ such that $(x^{k_1+1},\lambda^{k_1+1})$ satisfies the system
\begin{equation} \label{KKTQPemK0mais1}
\begin{array}{c}
Qx^{k_1+1}+c+A^T \lambda^{k_1+1}- \rho_{k_1}\max\left\{l-x^{k_1+1}+\frac{\mu^*_l}{\rho_{k_1}},0\right\}+
\rho_{k_1}\max\left\{x^{k_1+1}-u+\frac{\mu^*_u}{\rho_{k_1}},0\right\}=0, \\[5pt]
A x^{k_1+1}=b.
\end{array}
\end{equation}
...
Our tests are based on the \textsc{Netlib}~\cite{Dongarra:1987jk} test collection of linear programming problems, which consists of $98$ problems where, as reported in~\cite{Haeser:2017wl}, circa $65\%$ of them do not have strict interior, which indicates that those  are rather difficult problems. Hence, we used a presolved version of the collection available at~\cite{HagerCoapSoftware}. However, in our tests on linear programming problems, we found out that our implementation is too sensible to a fine tuning of the regularization parameter and the results were not satisfatory. Thus, we added a homogeneous quadratic term to the objective function with Hessian equal to the identity matrix, in order to obtain strictly convex separable quadratic problems. In this way, we may take the regularization parameter equal to zero at each iteration and we may use the second-order Lagrange multiplier update, which is easily computable. Also, the Newtonian direction can be accurately computed without refined linear solvers.
...
To analyze the results in this larger test, in \Cref{fig:Bar_LAQPNew-IPMMehrotra_All_2}, we present a ratio calculated for each of the $95$ available problems on the horizontal axis, ordered in increasing size of non-zero entries in the constraint matrix. A bar with a positive vertical coordinate indicates that \texttt{LAQP} is the best solver in terms of the number of linear systems solved, while a negative coordinate indicates that \texttt{IPM} is better. The size of the bar corresponds to the percentual difference in the number of linear systems solved by both solvers with respect to the worst one and it is given by the absolute value of $r_i$, for each problem $i$, computed as 
\[
...
We dedicate this paper in honor to the 70th birthday of Prof. J. M. Martínez, who greatly influenced our careers. Moreover, we would like to thank him for important discussions we had in designing the original ideas of this work. In addition, we would like to thank Profs. Roger Behling, Ernesto Birgin and Thadeu Senne for their discussions along different stages of the trajectory of this paper.
