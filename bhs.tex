...
$$L(x,\rho_k,\mu_l^k,\mu_u^k)= \frac{1}{2}x^TQx+\sum_{i=1}^n L_i^k(x_i),$$
where
\begin{equation*}\label{eq:defLik}
L_i^k(x_i)=c_ix_i+\frac{\rho_k}{2}\left(\max\left\{l_i-x_i+\frac{[\mu_l^k]_i}{\rho_k},0\right\}^2+
\max\left\{x_i-u_i+\frac{[\mu_u^k]_i}{\rho_k},0\right\}^2\right).
\end{equation*}
...
Combining \eqref{eq3xkcompacto} and  \eqref{eq1xkcompacto} we will show that $L(x^k,\rho_k,\mu_l^k,\mu_u^k)$ is significant greater than  $L(\bar{x},\rho_k,\mu_l^k,\mu_u^k)$. In order to do this we will show that,   given any $p>0$, for $x_1$ large enough
